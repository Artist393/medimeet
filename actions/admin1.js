...
export async function getPendingDoctors() {
  const isAdmin = await verifyAdmin();      //checks whether user is admin
...
export async function updateDoctorStatus(formData) {
  ...
  if (!doctorId || !["VERIFIED", "REJECTED"].includes(status)) {    //if status is neither verified or rejected
...
export async function updateDoctorActiveStatus(formData) {
  ...
  const doctorId = formData.get("doctorId");
  const suspend = formData.get("suspend") === "true";       //if the admin has suspended the doctor
